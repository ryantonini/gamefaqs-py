...
class ParsePlatformId(Parser):
    ...
    def __init__(self, url="http://www.gamefaqs.com/features/rankings"):
        ...
        self.platforms = open('platforms.txt', 'r').read().split('\n')
...
class ParseGameList(Parser):
    """Parse webpage(s) for game urls corresponding to the platform id specified.
    ...
    def get_games(self):
        """Get the urls of all the games on the current webpage."""
        ...
        for tag in tags:
            if tag.findChildren("a"):
                ...
                if atag.has_attr("href") and atag.contents:
                    self.game_urls[atag.contents[0]] = URL + atag['href']
